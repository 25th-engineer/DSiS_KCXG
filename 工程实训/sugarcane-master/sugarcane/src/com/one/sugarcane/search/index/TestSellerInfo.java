...
public class TestSellerInfo {
	...
	public void getResults() {
		...
		try {
			...
			rs = stmt.executeQuery("select * from sellerinfo");// 发送执行，获得结果集
			while (rs.next()) {// 遍历结果
				HashMap map = new HashMap();
				map.put("sellerId", rs.getString(1));
				map.put("sellerName", rs.getString(2));
				map.put("brief", rs.getString(3));
				this.dbList.add(map);
			}
	...
	public void createIndex(IndexWriter writer) {
		...
		for (Iterator it = this.dbList.iterator(); it.hasNext();) {
			...
			doc.add(new TextField("sellerId", map.get("sellerId").toString(), Field.Store.YES));
			doc.add(new TextField("sellerName", map.get("sellerName").toString(), Field.Store.YES));
			doc.add(new TextField("brief", map.get("brief").toString(), Field.Store.YES));
